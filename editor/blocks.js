        ...
        if (isV2) {
          const appendLineBreak = () => {
            if (typeof this.appendEndRowInput === "function") {
              ...
              // Fallback for older Blockly versions without EndRowInput.
          ...
          if (inlineOrder) {
            ...
            inlineQueues.forEach(queue => queue.forEach(item => orderedInline.push(item)));

            let inlineIdx = 0;
            const reordered = [];
            baseItems.forEach(item => {
              if (item && item.name && inlineNameSet.has(item.name)) {
                while (inlineIdx < orderedInline.length) {
                  const nextItem = orderedInline[inlineIdx++];
                  reordered.push(nextItem);
                  if (nextItem === item || nextItem?.name === item.name) break;
                }
              } else {
                reordered.push(item);
              }
            ...
            while (inlineIdx < orderedInline.length) {
              reordered.push(orderedInline[inlineIdx++]);
            }
            merged = reordered;
          }
